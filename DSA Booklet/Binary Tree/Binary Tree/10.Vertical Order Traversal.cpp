...
void VerticalOrder(Node *root, vi &v)
{
    if (!root)
        return;
    queue<State> q;
    ...
    while (!q.empty())
    {
        ...
        State curr = q.front();
        q.pop();
        max_dist = max(max_dist, curr.hd);
        min_dist = min(min_dist, curr.hd);
        vii temp = mp[curr.hd];
        temp.push_back({curr.node->data, curr.vd});
        int last = temp.size() - 1;
        while (last > 0 && temp[last].second == temp[last - 1].second && temp[last].first < temp[last - 1].first)
        {
            swap(temp[last], temp[last - 1]);
        }
        mp[curr.hd] = temp;
        if (curr.node->left)
        {
            State newState(curr.node->left, curr.hd - 1, curr.vd + 1);
            q.push(newState);
        }
        if (curr.node->right)
        {
            State newState(curr.node->right, curr.hd + 1, curr.vd + 1);
            q.push(newState);
        }
    }
