...
void LR(Node *root, vector<int> &lrv, int currLevel, int &maxLevel)
{
    if (!root)
        return;
    if (currLevel > maxLevel)
    {
        lrv.push_back(root->data);
        maxLevel = currLevel;
    }
    LR(root->left, lrv, currLevel + 1, maxLevel);
    LR(root->right, lrv, currLevel + 1, maxLevel);
}

void lR_view(Node *root, vector<int> &lrv)
{
    if (!root)
        return;
    queue<Node *> q;
