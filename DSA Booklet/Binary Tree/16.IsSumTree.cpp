...
struct state
{
    int isSumTree = 0;
    int sum = 0;
    state(int _isSumTree, int _sum)
    {
        isSumTree = _isSumTree;
        sum = _sum;
    }
};

state helper(Node *root)
{
    state newState(0, 0);
    if (!root)
    {
        newState.isSumTree = 1;
        newState.sum = 0;
        return newState;
    }
    ...
    {
        newState.isSumTree = 1;
        newState.sum = root->data;
        return newState;
    }
    state left = helper(root->left);
    state right = helper(root->right);
    newState.sum = root->data + left.sum + right.sum;
    bool check = (root->data == (left.sum + right.sum));
    if (check && left.isSumTree && right.isSumTree)
        newState.isSumTree = 1;
    return newState;
}
