...
int x,x1,y,y1;
if(a_contribution==0) x=INT32_MIN; else x=a[a_contribution-1];
if(a_contribution==m) x1=INT32_MAX; else x1=a[a_contribution];
if(b_contribution==0) y=INT32_MIN; else y=b[b_contribution-1];
if(b_contribution==n) y1=INT32_MAX; else y1=b[b_contribution];
cout << x <<  " " << x1 << " " << y << " " << y1 << "\n";
if(x<=y1 && y<=x1){
if((m+n)&1)
...
else if(y>x1) start=a_contribution+1;
else if(x>y1) end=a_contribution-1;
}
