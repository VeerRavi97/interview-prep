...
Node *dfs(vector<int> &preorder, int &pre_idx, int l, int r)
{
    if (pre_idx >= preorder.size())
        return nullptr;
    int curr = preorder[pre_idx];
    if (l < curr && curr < r)
    {
        Node *newNode = new Node(curr);
        pre_idx++;
        newNode->left = dfs(preorder, pre_idx, l, newNode->data);
        newNode->right = dfs(preorder, pre_idx, newNode->data, r);
        return newNode;
    }
    return nullptr;
}
...
Node *bstFromPreorder(vector<int> &preorder)
{
    ...
    int r = INT_MAX;
    int l = INT_MIN;
    return dfs(preorder, pre_idx, l, r);
}
