...
void addNodeToHead(Node *refer){
 ...
 int get(int key){
    int res=-1;
 if(mp.find(key)!=mp.end()){ // key found in cache
...
mp[key]=MRUelement;
if(count<cachesize){ // if cache is not full
count++;
...
cache.put(6,50);
cache.display();
cout << "the value of key 1:" << cache.get(1); cout << endl;
cache.display();

cout << "the value of key 3:" << cache.get(3); cout << endl;
cache.display();
cache.put(1,50);
cache.display();
cache.put(12,17);
...
cache.put(5,20);
cache.display();
cout << "the value of key 12:" << cache.get(12); cout << endl;
cache.display();
cout << "the value of key 4:" << cache.get(4); cout << endl;
cache.display();
