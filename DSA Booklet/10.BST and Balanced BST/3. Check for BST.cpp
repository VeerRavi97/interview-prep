...
bool dfs(Node *root, long long l, long long r)
{
    if (!root)
        return true;
    bool cond1 = (l < root->data && root->data < r);
    if (!cond1)
        return false;
    bool cond2 = dfs(root->left, l, root->data);
    bool cond3 = dfs(root->right, root->data, r);
    return cond1 && cond2 && cond3;
}

bool isValidBST(Node *root)
{
    long long int_max = (1LL << 33);
    long long int_min = -int_max;
    return dfs(root, int_min, int_max);
}

bool helper(Node *root, long long &prev)
{
    if (!root)
        return true;
